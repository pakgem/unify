$(document).ready(function () {
  ...
  function getAPICount() {
    var apiCount = localStorage.getItem("apiHits_" + anonymousID);
  ...
  function incrementAPICount() {
    ...
    localStorage.setItem("apiHits_" + anonymousID, currentCount + 1);
  }
  ...
  function initializeForm() {
    // Use a more specific selector for the form in the notes-gen section
    $(".notes-gen_wrap .default-email_form")
      .off("submit")
      .on("submit", function (e) {
        ...
        // Open get-started in new tab and prevent any navigation in current tab
  ...
  function checkAPILimit() {
    // Never show maxed state if user has submitted email
    ...
    if (apiCount >= 5) {
      // Changed back to >= since we're now using the correct counter
      showMaxedState();
      return true;
    } else {
      hideMaxedState();
  ...
  $(document).ready(function () {
    // Wait a tick to ensure our handler runs after global footer code
    setTimeout(function () {
      initializeForm();
      checkAPILimit();
    }, 0);
